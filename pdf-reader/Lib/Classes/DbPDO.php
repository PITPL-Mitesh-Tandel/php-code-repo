<?php
  ...
  namespace Pseudocode\PDFReader\Lib\Classes;
  ...
  class DbPDO {
    ...
    private $port;                 // Database server port
    ...
    public function connect($host='', $user='', $pw='', $db='', $port='') {
      ...
      switch($this->dbtype) {
        case 'pgsql':
          $dsn = $this->dbtype.':dbname='.$this->db.';port='.$this->port.';host='.$this->host;
          break;
        case 'sqlite':
      ...
      try {
        ...
        die('Connection ERROR: Code '.$e->getCode());
      }
    ...
    */
    private function getColumnNames($table)
	{
		switch ($this->dbtype) {
			case 'pgsql':
				$whereStr = " AND table_catalog = '".$this->db."' AND table_schema = 'public'";
				break;
			default:
				$whereStr = " AND table_schema = '".$this->db."'";
				break;
		}
		
		$sql = "SELECT column_name, data_type
				FROM information_schema.columns
				WHERE table_name = '".$table."'".$whereStr;
			
		try {
			$columnData = $this->getResults($sql); //pr($columnData);
        ...
        return false;
			...
			foreach ($columnData as $array) {
				foreach ($array as $innerKey => $value) {
					...
					if ($innerKey === 'column_name' && !(int)$innerKey) {
						$field = $value;
					}
					
					if ($innerKey === 'data_type' && !(int)$innerKey) {
    ...
    */
    ...
    public function printLastError($show_query = true) {
      ...
      if ($show_query && (!empty($this->lastQuery)) && $this->displayErrors) {
        $this->printLastQuery();
      }
