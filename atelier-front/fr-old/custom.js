...
(function () {
  ...
  factory('publicationsFactory', ['$state', 'modelFactory', 'Slug', function ($state, modelFactory, Slug) {
    ...
    Publications.prototype.go = function () {
      $state.go('pages-publications-show', { article: this['title-slug'] });
    };
...
(function() {
    ...
    controller('MainController', ['$scope', '$rootScope', '$state', '$timeout', '$http', 'studiosResolverFactory','$location', function($scope, $rootScope, $state, $timeout, $http, studios,$location) {
        $scope.studios = null;
            ...
            $timeout(function() {
                ...
                $scope.links = [
                   ...
                   // {state: 'pages-think-index', 'title': 'Think', background: $scope.colors.think},
                    ...
                    {state: 'pages-publications', 'title': 'Publications', background: $scope.colors.studios,color:$scope.colors2.studios},
...
(function () {
  ...
  workResolverFactory.$inject = ['$http', '$state', 'contentFormatter', 'workFactory', 'workCollectionFactory','$stateParams'];
  function workResolverFactory($http, $state, contentFormatter, Work, WorkCollection,$stateParams) {
    var factoryObj = {};
    ...
    factoryObj.getAll = function () {
      return $http({method: 'GET', cache: true, url: 'http://www.appdemo.co.in/atelier/content/0.1/get-work'})
        .then(function (result) {
          var formatted = {};
    ...
    factoryObj.getAllNotHidden = function () {
     ...
     factoryObj.getworkcategory = function ($id) {
      return $http({method: 'GET', cache: true, url: 'http://www.appdemo.co.in/atelier/content/0.1/get-work?tag='+$id})
        .then(function (result) 
        {
          var formatted = {};
        //  console.log($id);
  ...
  function WorkCategoryController($scope, work,$stateParams) {
    $scope.$on('animate.ready', function () {
     ...
     // $scope.setBreadcrumbs([{ 'title': 'WORK', 'ref': 'pages-work-category', 'color': 'work' }]);
                $scope.setBreadcrumbs([{ 'title': 'WORK', 'ref': 'pages-work-category', 'color': 'work' }, { 'title': $stateParams.id, 'color': '' }]);
...
(function () {
  ...
  WorkItemController.$inject = ['$http','$scope', '$timeout', '$stateParams', 'workResolverFactory', 'preloaderService']
  function WorkItemController($http,$scope, $timeout, $stateParams, work, PreloaderService) {
    $scope.$on('animate.ready', function () {
      $scope.$emit('animate.init');
      ...
      work.get($stateParams.id)
        .then(function (data) { 
          ...
          $scope.setBreadcrumbs([{ 'title': 'WORK', 'ref': 'pages-work-index', 'color': 'work' }, { 'title': $scope.work.title, 'color': '' }]);
              $scope.setHeaderDisplay(false); 
              console.log($scope.headerDisplay);
        ...
        .then(function (data) 
        {
          ...
          $scope.related.data = response.data;  //console.log($scope.related);
...
(function () {
  ...
  module.controller('PublicationsItemController', [
    ...
    function($scope, $stateParams, publiications) {
      var controller = this;
      $scope.$on('animate.ready', function () {
        $scope.$emit('animate.init');
...
(function (window) {
  ...
  var relativeUrlRegex = /^(\/)?(work|think|news|careers|about|studios|contact|publications)/;
...
(function(window) {
    ...
    window.app.run(['$rootScope', '$state', '$urlRouter', function($rootScope, $state, $urlRouter) {
        $rootScope.$on('$stateChangeSuccess', function(event, toState) {
            $rootScope.pagerContent = toState.name.match(/^news|publications|pages-home|pages-work-index|pages-studios-index/) ? true : false;
