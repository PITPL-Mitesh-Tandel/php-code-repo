...
(function() {
    ...
    SearchFactory.$inject = ['$state', '$stateParams', '$rootScope', 'modelFactory'];
...
(function () {
  ...
  var homeResolverFactory = function ($http,$rootScope, $stateParams, Hero, News, Studio, Think, Work, Career, Grid, Twitter, Publications)
  {
...
//    var url="http://www.atelierdada.com/atelier/content/0.1/get-index?lang=en";
  var url="http://www.atelierdada.com/atelier/content/0.1/get-index?lang=en";
  ...
  var url="http://www.atelierdada.com/atelier/content/0.1/get-index?lang=fr";
...
(function () {
  ...
  angular.module('dada.home').controller('HomeController', [
    ...
    '$scope', '$timeout', 'homeResolverFactory', '$stateParams', '$http',
    ...
    function($scope, $timeout, home,$stateParams,$http)
    {
      ...
      $timeout(function () {
        ...
        .then(function (data)
        {
         ...
         $http.get("http://www.atelierdada.com/atelier/content/0.1/get-testimonials?lang="+$stateParams.lang).then( function(response)
         {
...
//console.log($scope.testimonials);
          ...
          $scope.setPagedescription("We create imaginative lighting design solutions for architecture, landscape and urbanism, where space becomes \"made of light\"! \"Out-of the Box and state of the Art lighting design solutions for the inclusive and sustainable city of tomorrow\"");
          $scope.setPagetitle("Smart lighting design");
          if($stateParams.lang=='fr')
          {
              $scope.setPagedescription("Nous créons des solutions d’éclairage imaginatives pour l’architecture, le paysage et l’urbanisme, pour que l’espace soit « fait de lumière » ! \"Des solutions d'éclairage novatrices et de pointe pour la ville inclusive et durable de demain\"");
              $scope.setPagetitle("L'éclairage intelligent");
...
(function () {
  ...
  function workResolverFactory($http, $state, $stateParams, contentFormatter, Work, WorkCollection) {
    ...
    factoryObj.getAll = function () {
      return $http({method: 'GET', cache: true, url: 'http://www.atelierdada.com/atelier/content/0.1/get-work?lang='+$stateParams.lang})
        .then(function (result) {
          var formatted = {};
    ...
    factoryObj.getAllNotHidden = function () {
      return $http({method: 'GET', cache: true, url: 'http://www.atelierdada.com/atelier/content/0.1/get-work?featured=1&lang='+$stateParams.lang})
        .then(function (result) {
          var formatted = {};
     ...
     {
      return $http({method: 'GET', cache: true, url: 'http://www.atelierdada.com/atelier/content/0.1/get-work?tag='+$id+'&lang='+$stateParams.lang})
        .then(function (result)
        {
          var formatted = {};
        //  console.log($id);
            ...
            for (var j = 0; j < data.content.length; j++) 
            {
              ...
              if (typeof data.content[j].data.Video !== "undefined")
               ...
               // data.content[j].data.Video = "//player.vimeo.com/video/" + data.content[j].data.Video + "?title=0&amp;byline=0&amp;portrait=0&amp;badge=0";
                 ...
                 data.content[j].data.Video =  data.content[j].data.Video + "?title=0&amp;byline=0&amp;portrait=0&amp;badge=0";
                  data.content[j].data.originalurl =  data.content[j].data.Video + "?title=0&amp;byline=0&amp;portrait=0&amp;badge=0" 
           }
            break;
          }
        }
...
(function () {
  ...
  function WorkController($scope, work,$stateParams,$window,$document) {
    $scope.$on('animate.ready', function () {
      ...
      {
          $scope.setBreadcrumbs([{ 'title': 'Projets', 'ref': 'pages-work-index', 'color': 'work' },{ 'title': 'Typologie', 'ref': 'pages-work-index', 'color': 'work','display_menu':'1' }]);
      }
      ...
      if($stateParams.lang=='en')
      {
         ...
         $scope.setPagedescription("Our projects featured here give a flavor of the best, newest and most innovative lighting design work.");

      }
      else
      {
           $scope.setPagetitle('Zoom projets');
           $scope.setPagedescription("Noss projets présentés ici donnent un aperçu du meilleur, du plus récent et du plus innovant travail de conception lumière.");
      }
...
//console.log($scope.work['night-cover']);
     ...
     //  $scope.member1 = "http://www.appdemo.co.in/atelier/file-manager/content/4063033833478374_1485159434_203665.jpg";
      $scope.member1 = "http://www.atelierdada.com/atelier/file-manager/content/"+$scope.work['night-cover'];
       ...
       $scope.ori_team_image1 = "images/marie-about.jpg";
        ...
        else if( $stateParams.lang=='fr' && $state.current.name=='pages-work-show')
        {
         // "pages-home({lang: 'en'})
          ...
          {
             $scope.setPagetitle($scope.work.title+" | "+ $scope.work.location);
          }
        ...
        .then(function (data)
        {
           $http.get("http://www.atelierdada.com/atelier/content/0.1/get-related-work?slug="+$stateParams.id+"&lang="+$stateParams.lang).then( function(response)
          {
            $scope.related = {};
...
(function () {
 ...
 module.controller('AboutshowController', ['$scope','$http', '$timeout', '$stateParams', 'aboutResolverFactory', 'preloaderService', function ($scope,$http, $timeout, $stateParams, about, PreloaderService) 
  {
      ...
      $scope.$on('animate.ready', function () 
      {
       ...
       //   $scope.setBreadcrumbs([{ 'title': 'ABOUT' }]);
            $scope.setBreadcrumbs([{ 'title': 'ABOUT', 'ref': 'pages-about-index({lang:\'en\'})' }, { 'title': title, 'color': '' }]);
          }
          else{
             // $scope.setBreadcrumbs([{ 'title': 'ATELIER' }]);
               $scope.setBreadcrumbs([{ 'title': 'ATELIER', 'ref': 'pages-about-index({lang:\'fr\'})' }, { 'title': title, 'color': '' }]);
          }
         ...
         $scope.iald_link = "#";
          ...
          if($stateParams.id=='nirmit-jhaveri')
          {
               //$scope.imgsrc = "images/nirmit-about.jpg";
                ...
                $scope.linkedin_link  = "https://www.linkedin.com/in/nirmit-jhaveri-8a95423?ppe=1";
               if($stateParams.lang=='en')
               {
                   //var url = "backend/api/about-nirmit.json";
                var url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-nirmit&lang=en";

                $scope.setPagetitle("Nirmit Jhaveri | Lighting design Principal of Atelier dada");
                $scope.setPagedescription("Nirmit comes with strong professional background in architecture, urban design and interior design prior to specializing in lighting design.");
               ...
               else
               {
                  // var url = "backend/api/about-nirmit-fr.json";
                   ...
                   var url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-nirmit&lang=fr";
                    $scope.setPagetitle("Nirmit Jhaveri | Concepttrice lumière Principale de l'Atelier dada");
                    $scope.setPagedescription("Concepttrice lumière Principale de l'Atelier dada");
               }
            
          
          }
          if($stateParams.id=='marie-ikram-bouhlel-jhaveri')
          {
                $scope.imgsrc = "images/marie-about.jpg";
               ...
               //  var url = "backend/api/about-marie.json";
                $scope.linkedin_link  = "https://in.linkedin.com/in/marieikrambouhlel";
                 $scope.ace_link = "http://www.ace-fr.org/membres/concepteur/actif-senior/bouhlel-marie-ikram/";
                  $scope.viado_link = "http://fr.viadeo.com/fr/profile/ikram.bouhlel";
                  $scope.iald_link = "https://www.iald.org/Designer-Directory/Designers/Marie-Ikram-Bouhlel";


              if($stateParams.lang=='en')
               {
                     var url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-marie&lang=en";
                //   var url = "backend/api/about-marie.json";
                   

                     $scope.setPagetitle("Marie Ikram Bouhlel Jhaveri | Lighting design Principal of Atelier dada");
                     $scope.setPagedescription("Lighting design Principal of Atelier dada");
               }
               else
               {
                    var url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-marie&lang=fr";
                   //var url = "backend/api/about-marie-fr.json";
                    $scope.setPagetitle("Marie Ikram Bouhlel Jhaveri | Concepteur lumière Principal de l'Atelier dada");
                   $scope.setPagedescription("Concepteur lumière Principal de l'Atelier dada");
               }
    ...
    $scope.$on('animate.ready', function () 
    {
      $scope.$emit('animate.init');
          ...
          else
          {
           ...
           if($stateParams.lang=='en')
           {
             $scope.setPagedescription("To understand more about our expertise and design approach, please don't hesitate to contact us for a chat.");
               $scope.setPagetitle("Multi-award winning Innovative lighting design professionals");
           ...
           {
               $scope.setPagetitle("Professionnels de l'éclairage novateurs et primés");
             $scope.setPagedescription("Pour en savoir plus sur notre expertise et approche conceptuelle, n'hésitez pas à nous contacter pour en discuter.");
/*
      ...
      //  $scope.default_img = response.background;
         ...
         $scope.member1 = response.content[4]['data']['OneColumnImage'];//. "images/contact-us/yagnesh.jpg";
...
(function () {
  ...
  module.factory('aboutResolverFactory', ['$http', '$state', '$stateParams','contentFormatter', 'aboutFactory', function ($http, $state, $stateParams, contentFormatter, About) {
    ...
    factoryObj.getContent = function ()
    {
      ...
      var url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-us&lang=en";
   
      if($stateParams.lang=='fr')
      {
     //   url = "backend/api/about-us-fr.json";
         url  = "http://www.atelierdada.com/atelier/content/0.1/get-page?slug=about-us&lang=fr";
      }
      ...
      var controller = this;
       $scope.$on('animate.ready', function () {
        ...
        if($stateParams.lang=='en')
        {
          ...
          $scope.setPagedescription("The latest news and press coverage about Atelier dada.");
        }
       else
       {
         $scope.setPagetitle("Dernières Nouvelles");
         $scope.setPagedescription("Les dernières actualités et publications sur l'Atelier dada.");
       }
...
(function () {
  ...
  module.factory('newsResolverFactory', ['$http', '$stateParams', 'newsFactory', function($http, $stateParams, News) {
    ...
    return {
      getAll: function ()
      {
        return $http({method: 'GET', cache: true, url: 'http://www.atelierdada.com/atelier/content/0.1/get-contents?type=n&lang='+$stateParams.lang})
          .then(function (result) 
          {
...
(function () {
  ...
  if($stateParams.lang=='en')
  {
   ...
   $scope.setPagedescription("Read more about awards we won across the globe and recent publications.");
  }
  else
  {
    $scope.setPagetitle("Prix Internationaux | Média |  Presse ");
    $scope.setPagedescription("En savoir plus sur les publications récentes et les prix que nous avons remportés à travers le monde.");
  }
        var onLoad = function(data) {
...
(function () {
  ...
  module.factory('publicationsResolverFactory', ['$http','$stateParams', 'publicationsFactory', function($http, $stateParams,Publications) {
    ...
    return {
      getAll: function () {
        return $http({method: 'GET', cache: true, url: 'http://www.atelierdada.com/atelier/content/0.1/get-contents?type=c&lang='+$stateParams.lang})
          .then(function (result) {
            var promoted = result.data.filter(function(item) {
              return item.promoted == true;
            })
...
(function() {
  ...
  studiosResolverFactory.$inject = ['$http','$stateParams','studioFactory', 'studioCollectionFactory'];
  

  function studiosResolverFactory($http,$stateParams, Studio, StudioCollection) 
  {
...
(function () {
  ...
  function StudiosController($scope,$stateParams, $timeout, studios, PreloaderService, EventDetection) {
   ...
   $scope.setPagedescription("We operate from our unique studio in Paris/France.");
   }
   else
   {
     $scope.setPagetitle("Paris | France | Atelier dada");
    $scope.setPagedescription("Nous opérons  depuis notre unique atelier à Paris / France.");
   }
      ...
      $scope.showVideo = typeof EventDetection.getTransitionEvent() !== 'undefined';
       ...
       $scope.openmap = function(country) 
       {
        ...
        if(country=='Paris')
        {
         window.open('https://goo.gl/maps/uPVDEJE2uDm','_blank');
        }
        else
        {
...
(function() {
    ...
    function SearchResultsController($scope,$stateParams, $location, searchResolver) {
        $scope.resultsGrid = [];
...
(function () {
  ...
  module.controller('ContactController', ['$state','$scope','$stateParams', '$http', 'studiosResolverFactory', function ($state,$scope, $stateParams, $http, studios) {
    $scope.$on('animate.ready', function ()
    {
...
*/
      ...
      $scope.setBreadcrumbs([{ 'title': 'CONTACT', 'ref': 'pages-contact', 'color': 'contact' }]);
        ...
        if($stateParams.lang=='en')
        {
          $scope.setPagedescription("We operate from our unique studio in Paris/France. Give us a call, send us an email or drop by to have a chat. ");
          $scope.setPagetitle("Contact us for new ideas and initiatives in lighting design");
        }
        else
        {
           $scope.setPagedescription("Nous opérons à partir de notre unique studio à Paris / France. Appelez-nous, envoyez-nous un courriel ou passez nous voir pour discuter.");
           $scope.setPagetitle(" Contactez-nous pour des idées et initiatives novatrices en conception lumière");
        }
        ...
        .then(function (data) 
        {
          ...
          $scope.validate = function () {
            ...
            for (i in $scope.stages) {
              if ($scope.stages.hasOwnProperty(i) && $scope.stages[i] === true) {
                validate.push(i);
              }
            }
          ...
          $scope.show = function (id) {
            ...
            if ($scope.validate()) 
            {
              if (id === 'contact3') {
                ...
                $http.post('backend/sendemail.php', $scope.contact)
...
(function () {
  ...
  PrivacyController.$inject = [
    '$scope', '$timeout', 'privacyResolverFactory', 'preloaderService','$stateParams'
  ];

  function PrivacyController($scope, $timeout, privacy, PreloaderService,$stateParams) 
  {
     $scope.$on('animate.ready', function ()
    {
      privacy.getAll()
      ...
      //  Mentions légales
         ...
         if($stateParams.lang=='en')
         {
               $scope.setBreadcrumbs([{ 'title': 'Legal Notice', 'ref': 'pages-info-privacy', 'color': '' }]);
              $scope.setPagetitle("Legal Notice");
         }
         else
         {
                $scope.setBreadcrumbs([{ 'title': 'Mentions légales', 'ref': 'pages-info-privacy', 'color': '' }]);
                ...
                page.element.on(transitionEvent, function (event) 
                {
                 ...
                 console.log( $scope.page);
                   ...
                   console.log( "direction "+$scope.direction);
                     ...
                     checkPage = $scope.direction === DIRECTION_DOWN ? $scope.page : $scope.page + 1;
...
(function(window) {
    ...
    window.app.run(['$rootScope', '$state', '$urlRouter','$stateParams','$location', function($rootScope, $state, $urlRouter,$stateParams,$location)
     {
            ...
            $rootScope.$on('$locationChangeSuccess', function() 
            {
               ...
               var part_url = $location.path().split('/');
                ...
                if(part_url[1]=='fr')
                {
                    ...
                    $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/urbain', '/category/urban');
                  $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/r%C3%A9sidentiel', '/category/residential');
                    $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/institutionnel', '/category/institution');
                    $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/h%C3%B4telier', '/category/hospitality');
                    $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/ev%C3%A8nementiel', '/category/event');
                    $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/produit', '/category/product');
                  $rootScope.french_url = $location.absUrl();   //no change in french url
                ...
                else
                {
                  ...
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/urban', '/category/urbain');
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/residential', '/category/r%C3%A9sidentiel');
              
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/institution', '/category/institutionnel');
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/hospitality', '/category/h%C3%B4telier');
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/event', '/category/ev%C3%A8nementiel');
                  $rootScope.french_url  =   $rootScope.french_url.replace('/category/product', '/category/produit');
          ...
          if(part_url[1]=='fr')
          {
           ...
           //   $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/commercial', '/category/commercial');
              $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/institutionnel', '/category/institution');
              $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/hôtelier', '/category/hospitality');
              $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/ev%C3%A8nementiel', '/category/event');
              $rootScope.eng_url  =   $rootScope.eng_url.replace('/category/produit', '/category/product');
