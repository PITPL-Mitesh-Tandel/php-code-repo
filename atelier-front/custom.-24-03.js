  ...
  function WorkItemController($http,$scope, $state,$timeout, $stateParams, work, PreloaderService) {
    $scope.$on('animate.ready', function () {
      ...
      work.get($stateParams.id)
        ...
        if($stateParams.id=="sumel-6" ||  $stateParams.id=="mondeal-square-facade"  || $stateParams.id=="mondeal-square-landscape" || $stateParams.id=="sumel-3" || $stateParams.id=="mondeal-heights" )
        {
           ...
           if($stateParams.id=="shivalik-hs" )
        {
          ...
          if($stateParams.id=="ccbta-terre-d-argence" )   //put in goussainvile also
        {
         ...
         if($stateParams.id=="ppp-goussainville" )   //put in goussainvile also
        {
          $scope.testimonial_client = "Jean Sabatier";
          $scope.testimonial_studio = "Lumières Urbaines, Ancien Président de l’ACE";
          $scope.testimonial_client_speech = "Naissance d'une star! <br/>J'ai mis à l'épreuve sa sensibilité, son sens artistique et sa perception du besoin des habitants en terme d'identification des espaces, de sécurité et d'amélioration du cadre de vie. Mme Bouhlel a exprimé avec une remarquable justesse les lignes fortes d'un programme d'aménagement lumière, parfaitement reconnu par les élus et admis par les services techniques.Après un travail d'analyse d'une grande précision, elle a montré avec clarté les tendances, les équilibres, les variations propres à valoriser sans excès un milieu urbain de nature aussi différenciée, à la grande satisfaction de tous les acteurs.";
  ...
  module.controller('ContactController', ['$state','$scope','$stateParams', '$http', 'studiosResolverFactory', function ($state,$scope, $stateParams, $http, studios) {
    $scope.$on('animate.ready', function ()
    {
      ...
      //  console.log(title);
        ...
        {
          $scope.isahmedabadSelected  = true;
        $scope.isparisSelected  = false;
        }
