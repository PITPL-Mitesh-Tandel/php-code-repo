...
(function () {
  ...
  factory('newsFactory', ['$state', 'modelFactory', 'Slug', function ($state, modelFactory, Slug) {
    function News() {
    }
    ...
    News.prototype.go = function () {
      $state.go('pages-news-show', { article: this['title-slug'] });
    };
...
(function() {
    ...
    factory('workFactory', ['$state', 'modelFactory', 'Slug', function($state, modelFactory, Slug) {
        function Work() {
        }
        ...
        Work.prototype.go = function() {
            $state.go('pages-work-show', { id: this['title-slug'] });
        };
...
(function() {
  var module = angular.module('fitch.components.related-links', []);
...
(function() {
    ...
    controller('MainController', ['$scope', '$rootScope', '$state', '$timeout', '$http', 'studiosResolverFactory', function($scope, $rootScope, $state, $timeout, $http, studios) {
        $scope.studios = null;
        ...
        // this is set in the run.js
        // if you change the value and nothing happens, that is why
        ...
        */    
        ...
        $scope.toggleSearch = function() {
            ...
            if ($rootScope.magToggle) {
                ...
                $state.go('controls-search.details');
            }
        };
...
(function() {
    ...
    controller('NavigationController', ['$scope', '$timeout', function($scope, $timeout) {
        $scope.$on('animate.ready', function() {
            $timeout(function() {
                ...
                $scope.links = [
                   ...
                   // {state: 'pages-think-index', 'title': 'Think', background: $scope.colors.think},
                    ...
                    {state: 'pages-studios-index', 'title': 'Media', background: $scope.colors.studios,color:$scope.colors2.studios},
...
(function() {
  var module = angular.module('fitch.home', [
    'ui.router',
    'models.custom.hero',
    'models.custom.news',
...
(function () {
  ...
  var factory = angular.module('fitch.home').factory;

  var homeResolverFactory = function ($http, Hero, News, Studio, Think, Work, Career, Grid, Twitter) {
    ...
    return {
      getAll: function (tweet) {
        return $http({method: 'GET', url: 'backend/api/index.json'})
...
(function () {
  ...
  angular.module('fitch.home').controller('HomeController', [
    
    '$scope', '$timeout', 'homeResolverFactory',
    
    function($scope, $timeout, home) {
      $timeout(function () {
...
(function() {
  var module = angular.module('fitch.work', [
    'models.custom.work',
...
(function () {
  ...
  workResolverFactory.$inject = ['$http', '$state', 'contentFormatter', 'workFactory', 'workCollectionFactory'];
  function workResolverFactory($http, $state, contentFormatter, Work, WorkCollection) {
    var factoryObj = {};
    ...
    factoryObj.getAll = function () {
      return $http({method: 'GET', cache: true, url: 'backend/api/work.json'})
        .then(function (result) {
          var formatted = {};
...
(function () {
  ...
  function WorkController($scope, work) {
    $scope.$on('animate.ready', function () {
      ...
      $scope.setBreadcrumbs([{ 'title': 'WORK', 'ref': 'pages-work-index', 'color': 'work' }]);
...
(function () {
  ...
  WorkItemController.$inject = ['$scope', '$timeout', '$stateParams', 'workResolverFactory', 'preloaderService']
  function WorkItemController($scope, $timeout, $stateParams, work, PreloaderService) {
    $scope.$on('animate.ready', function () {
      $scope.$emit('animate.init');
      ...
      work.get($stateParams.id)
        .then(function (data) {
          ...
          $scope.setBreadcrumbs([{ 'title': 'WORK', 'ref': 'pages-work-index', 'color': 'work' }, { 'title': $scope.work.client, 'color': '' }]);
...
(function() {
  var module = angular.module('fitch.about', [
    'ui.router',
    'models.custom.about'
  ]);
...
(function () {
  ...
  module.factory('aboutResolverFactory', ['$http', '$state', 'contentFormatter', 'aboutFactory', function ($http, $state, contentFormatter, About) {
    var factoryObj = {};

    factoryObj.getContent = function () {
      return $http({method: 'GET', cache: true, url: 'backend/api/about-us.json'})
...
(function() {
  var module = angular.module('fitch.news', []);
...
(function () {
  ...
  module.factory('newsResolverFactory', ['$http', 'newsFactory', function($http, News,$widows) {
    ...
    return {
      getAll: function () {
        return $http({method: 'GET', cache: true, url: 'backend/api/news.json'})
          .then(function (result) 
          {
...
(function() {
  var module = angular.module('fitch.think', [
    'ui.router',
...
(function() {
  var module = angular.module('fitch.studios', [
    'ui.router',
    'models.custom.studio',
...
(function () {
  ...
  StudiosController.$inject = ['$scope', '$timeout', 'studiosResolverFactory', 'preloaderService', 'EventDetection'];
  function StudiosController($scope, $timeout, studios, PreloaderService, EventDetection) {
    $scope.$on('animate.ready', function () {
      $scope.$emit('animate.init');
...
(function() {
  var module = angular.module('fitch.career', [
    'ui.router',
    'models.custom.career'
  ]);
...
(function() {
  var module = angular.module('fitch.search', [
    'ui.router',
...
(function() {
    angular.module('fitch.search').factory('searchResolverFactory', Search);
    ...
    function Search(Search) {
        ...
        var getAll = function(work, think, news) {
            if (data) {
                return data;
            }
            ...
            for (i = 0; i < think.length; i++) {
                searchObj = new Search();
                searchObj.populate('search', {model: think[i]});
                formatted.push(searchObj);
            }
        ...
        function cleanSearchTerms(term) {
            if (term) {
                return term.replace(/\s/g, '-').replace('&', 'and').toLowerCase();
            }
...
(function() {
    ...
    angular.module('fitch.search').controller('SearchController', SearchController);
    ...
    function SearchController($scope, $q, work, think, news) {
        $scope.data = [];
...
(function() {
    angular.module('fitch.search').controller('SearchResultsController', SearchResultsController);
...
(function() {
    ...
    angular.module('fitch.search').controller('SearchTermsController', SearchTermsController);
...
(function() {
  var module = angular.module('fitch.contact', [
    'ui.router',
    'fitch.studios'
  ]);
...
(function () {
  ...
  module.controller('ContactController', ['$scope', '$http', 'studiosResolverFactory', function ($scope, $http, studios) {
    $scope.$on('animate.ready', function () {
      $scope.$emit('animate.init');
          ...
          $scope.show = function (id) {
            ...
            if ($scope.validate()) {
              if (id === 'contact3') {
                ...
                $http.post('/backend/contact', $scope.contact)
                  .success(function () {
                    $scope.sending = false;
                    $scope.complete = true;
                  })
...
(function() {
  var module = angular.module('fitch.info', []);
...
(function (window) {
  ...
  var nonDomainRegex = /(http|https):\/\/(fitch\.app|stage\.fitch\.com|www\.fitch\.com)/;
  var relativeUrlRegex = /^(\/)?(work|think|news|careers|about|studios|contact)/;
...
(function(window) {
    ...
    var directive = window.angular.module('cergis.directives.squeeky-video', []).directive;

    directive('squeekyVideoContainer', [function($sce) {
        return {
            restrict: 'EA',
            controller: ['$scope', '$element', function($scope, $container) {
                var video;

                var cancelPlay = false;

                this.addVideo = function(element) {
                    var calculatedWidth = (element.width() - 640) / 2;
                    var position = '-' + (calculatedWidth > 0 ? calculatedWidth : 0) + 'px';
    ...
    directive('squeekyHoverTarget', [function($sce) {
        return {
            restrict: 'A',
            require: '^squeekyVideoContainer',
            link: function(scope, element, attrs, controller) {
                element.on('mouseenter', function() {
                    controller.play();
                });
    ...
    directive('squeekyVideo', ['$sce', function($sce) {
        return {
            restrict: 'E',
            replace: true,
            require: '^squeekyVideoContainer',
            template: '<video loop webkitallowfullscreen mozallowfullscreen allowfullscreen><source data-src="{{src}}" type="video/mp4"></video>',
            scope: {
                'videoSrc': '@'
            },
            link: function(scope, element, attrs, controller) {
                scope.src = $sce.trustAsResourceUrl(scope.videoSrc);
                controller.addVideo(element);
            }
        }
    }]);
})(window);
...
(function(window) {
    ...
    var directive = window.angular.module('cergis.directives.auto-video', []).directive;

    directive('autoVideo', ['$sce', function($sce) {
        return {
            restrict: 'E',
            replace: true,
            template: '<video loop webkitallowfullscreen mozallowfullscreen allowfullscreen><source src="{{src}}" type="video/mp4"></video>',
            scope: {
                'videoSrc': '@'
            },
            link: function(scope, element, attrs, controller) {
...
(function() {
  ...
  var customModules = [
    ...
    'fitch.components.related-links',
    'fitch.home',
    'fitch.think',
    'fitch.studios',
    'fitch.search',
...
(function(window) {
    ...
    window.app.run(['$rootScope', '$state', '$urlRouter', function($rootScope, $state, $urlRouter) {
